...
template<typename T, int BLOCK_TILE_M, int BLOCK_TILE_N, int WARP_TILE_M, int WARP_TILE_N>
__global__ void fp16_mma_m16n8k16_ldmatrix_trans__overlap_global_2_sm__opt(const T* A, const T* B, const T* C, int M, int N, int K)
{
  constexpr int WARP_COUNT   = BLOCK_TILE_M / WARP_TILE_M * BLOCK_TILE_N / WARP_TILE_N;
  constexpr int THREAD_COUNT = WARP_COUNT * 32;
...
#define sm_2_mma_reg(ldg_buffer_index, group)                                                                          \
  {                                                                                                                    \
    {                                                                                                                  \
      uint32_t src =                                                                                                   \
        __cvta_generic_to_shared(&A_sm[ldg_buffer_index][lane_id / 8][m_warp_offset + (group) * 8 + lane_id % 8][0]);    \
    ...
    {                                                                                                                  \
      uint32_t src =                                                                                                   \
        __cvta_generic_to_shared(&B_sm[ldg_buffer_index][lane_id % 16 / 8]                                             \
                                      [(n_warp_offset + group * 16 + lane_id / 16 * 8) / 8][lane_id % 8][0]);          \
      asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 {%0, %1, %2, %3}, [%4];"                            \
                   : "=r"(*(uint32_t*)&B_mma_reg[group][0]),                                                           \
                     "=r"(*(uint32_t*)&B_mma_reg[group][4]),                                                           \
                     "=r"(*(uint32_t*)&B_mma_reg[group][2]),                                                           \
  ...
  k_loop_offset = LOOP_TILE_K;
  global_2_ldg_reg();
  while (k_loop_offset + LOOP_TILE_K < K) {
    for (int group = 0; group < M_GROUP_COUNT_PER_WARP; ++group) {
      uint32_t src =
        __cvta_generic_to_shared(&A_sm[LDG_BUFFER_INDEX][lane_id / 8][m_warp_offset + group * 8 + lane_id % 8][0]);
      asm volatile("ldmatrix.sync.aligned.m8n8.x2.shared.b16 {%0, %1}, [%2];"
...
#define sm_2_B_mma_reg(ldg_sm_buffer_index, mma_reg_buffer_index)                                                      \
  for (int group = 0; group < N_GROUP_COUNT_PER_WARP; ++group) {                                                       \
    uint32_t src =                                                                                                     \
      __cvta_generic_to_shared(&B_sm[ldg_sm_buffer_index][lane_id % 16 / 8]                                            \
                                    [(n_warp_offset + (group) * 16 + lane_id / 16 * 8) / 8][lane_id % 8][0]);          \
    asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 {%0, %1, %2, %3}, [%4];"                              \
...
define_check_function(fp16_mma_m16n8k16_ldmatrix_trans__overlap_global_2_sm__opt);
         ...
         int                       K)
{
  ...
  fp16_mma_m16n8k16_ldmatrix_trans__overlap_global_2_sm__opt___check_relative_error(fp16_A, fp16_B, fp16_C, M, N, K, host_C);
