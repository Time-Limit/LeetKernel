...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void llmmm(const float *A, const float *B, float *C, int M, int N,
                     int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
  ...
  const int comp_thread_m_offset =
      threadIdx.x / CAL_THREADS_ON_N_AXIS * THREAD_TILE_M;
...
#pragma unroll
    for (int loop = 0; loop < A_LDG_LOOP_COUNT; ++loop) {
      ...
      FETCH_FLOAT4(A_ldg_reg[loop * 4],
                   A[OFFSET(block_m_offset + m, k_iter_offset + k, K)]);
    }
...
#pragma unroll
    for (int loop = 0; loop < B_LDG_LOOP_COUNT; ++loop) {
      ...
      FETCH_FLOAT4(B_ldg_reg[loop * 4],
                   B[OFFSET(k_iter_offset + k, block_n_offset + n, N)]);
    }
...
#pragma unroll
      for (int i = 0; i < CAL_THREAD_M_LOAD_COUNT; ++i) {
        FETCH_FLOAT4(A_comp_reg[i * 4],
                     A_sm[0][k][comp_thread_m_offset + i * CAL_THREAD_M_STRIDE]);
      }
      // Load B from shared to register
#pragma unroll
      for (int i = 0; i < CAL_THREAD_N_LOAD_COUNT; ++i) {
        FETCH_FLOAT4(B_comp_reg[i * 4],
                     B_sm[0][k][comp_thread_n_offset + i * CAL_THREAD_N_STRIDE]);
      }
#pragma unroll
...
#pragma unroll
    for (int j = 0; j < THREAD_TILE_N; j += 4) {
      const int n =
          block_n_offset + comp_thread_n_offset + j / 4 * CAL_THREAD_N_STRIDE;
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_sm2reg(const float *A, const float *B,
...
#pragma unroll
    for (int i = 0; i < CAL_THREAD_M_LOAD_COUNT; ++i) {
      FETCH_FLOAT4(A_comp_reg[0][i * 4],
                   A_sm[0][0][comp_thread_m_offset + i * CAL_THREAD_M_STRIDE]);
    }
#pragma unroll
    for (int i = 0; i < CAL_THREAD_N_LOAD_COUNT; ++i) {
      FETCH_FLOAT4(B_comp_reg[0][i * 4],
                   B_sm[0][0][comp_thread_n_offset + i * CAL_THREAD_N_STRIDE]);
    }
...
#pragma unroll
      for (int i = 0; i < CAL_THREAD_M_LOAD_COUNT; ++i) {
        FETCH_FLOAT4(
            A_comp_reg[(k + 1) & 1][i * 4],
            A_sm[0][k + 1][comp_thread_m_offset + i * CAL_THREAD_M_STRIDE]);
      }
#pragma unroll
      for (int i = 0; i < CAL_THREAD_N_LOAD_COUNT; ++i) {
        FETCH_FLOAT4(
            B_comp_reg[(k + 1) & 1][i * 4],
            B_sm[0][k + 1][comp_thread_n_offset + i * CAL_THREAD_N_STRIDE]);
      }
#pragma unroll
      for (int i = 0; i < THREAD_TILE_M; ++i) {
#pragma unroll
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_global2sm(const float *A, const float *B,
                                               float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
...
#pragma unroll
      for (int i = 0; i < CAL_THREAD_M_LOAD_COUNT; ++i) {
        FETCH_FLOAT4(A_comp_reg[i * 4],
                     A_sm[SM_CALCULATING_BUFFER][k]
                         [comp_thread_m_offset + i * CAL_THREAD_M_STRIDE]);
      }
      // Load B from shared to register
#pragma unroll
      for (int i = 0; i < CAL_THREAD_N_LOAD_COUNT; ++i) {
        FETCH_FLOAT4(B_comp_reg[i * 4],
                     B_sm[SM_CALCULATING_BUFFER][k]
                         [comp_thread_n_offset + i * CAL_THREAD_N_STRIDE]);
      }
#pragma unroll
      for (int i = 0; i < THREAD_TILE_M; ++i) {
#pragma unroll
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void llmmm__overlap_global2sm2reg__using_if(
...
#define compute()                                                          \
  _Pragma("unroll") for (int i = 0; i < THREAD_TILE_M; ++i) {                  \
    _Pragma("unroll") for (int j = 0; j < THREAD_TILE_N; ++j) {                \
      C_reg[i][j] += A_comp_reg[CAL_REG_CALCULATING_BUFFER][i] *                \
                     B_comp_reg[CAL_REG_CALCULATING_BUFFER][j];                 \
    }                                                                          \
  }
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void llmmm__overlap_global2sm2reg(
    const float *A, const float *B, float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_global2sm2reg__quardra_buffer(
    const float *A, const float *B, float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
...
#define compute()                                                              \
  _Pragma("unroll") for (int i = 0; i < THREAD_TILE_M; ++i) {                  \
    _Pragma("unroll") for (int j = 0; j < THREAD_TILE_N; ++j) {                \
      C_reg[i][j] +=                                                           \
          A_comp_reg[CAL_REG_COMPUTING][i] * B_comp_reg[CAL_REG_COMPUTING][j]; \
    }                                                                          \
  }
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_global2sm2reg__quardra_buffer__double_ldg_reg(
    const float *A, const float *B, float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_global2sm2reg__quardra_buffer__double_ldg_reg__st_global_wt(
    const float *A, const float *B, float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
...
#pragma unroll
    for (int j = 0; j < THREAD_TILE_N; j += 4) {
      ...
      asm volatile("st.global.wt.v4.f32 [%0], {%1, %2, %3, %4};"
                   :
                   : "l"(&C[OFFSET(m, n, N)]), "f"(C_reg[i][j]),
                     "f"(C_reg[i][j + 1]), "f"(C_reg[i][j + 2]),
                     "f"(C_reg[i][j + 3])
                   : "memory");
    }
  }
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_global2sm2reg__quardra_buffer__double_ldg_reg__st_global_wt__pipeline(
    const float *A, const float *B, float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
  ...
  {                                                                            \
    ...
    _Pragma("unroll") for (int loop = 0; loop < A_LDGSTS_LOOP_COUNT; ++loop) { \
      ...
      __pipeline_memcpy_async(                                                 \
          &A_sm[SM_LOADING_BUFFER][k][m],                                      \
          &A[OFFSET(block_m_offset + m, k_iter_offset + k, K)], sizeof(float), \
          0);                                                                  \
    }                                                                          \
    /* Load B, global->register */                                             \
    _Pragma("unroll") for (int loop = 0; loop < B_LDGSTS_LOOP_COUNT; ++loop) { \
      ...
      __pipeline_memcpy_async(                                                 \
          &B_sm[SM_LOADING_BUFFER][k][n],                                      \
          &B[OFFSET(k_iter_offset + k, block_n_offset + n, N)], sizeof(float), \
          0);                                                                  \
    }                                                                          \
  }
...
template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,
          int THREAD_TILE_N, int TILE_K>
__global__ void
llmmm__overlap_global2sm2reg__quardra_buffer__double_ldg_reg__st_global_wt__pipeline_PTX(
    const float *A, const float *B, float *C, int M, int N, int K) {
  constexpr int THREAD_COUNT =
      device_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,
                                     THREAD_TILE_N>();
...
#define global_2_sm()                                                          \
  {                                                                            \
    ...
    _Pragma("unroll") for (int loop = 0; loop < A_LDGSTS_LOOP_COUNT; ++loop) { \
      ...
      const int k = (loop * THREAD_COUNT + threadIdx.x) % TILE_K;              \
      uint32_t dst_ptr =                                                       \
          __cvta_generic_to_shared(&A_sm[SM_LOADING_BUFFER][k][m]);            \
      asm volatile(                                                            \
          "cp.async.ca.shared.global.L2::128B [%0], [%1], 4, 4;\n" ::"r"(      \
              dst_ptr),                                                        \
    ...
    _Pragma("unroll") for (int loop = 0; loop < B_LDGSTS_LOOP_COUNT; ++loop) { \
      ...
      const int n = (loop * THREAD_COUNT + threadIdx.x) % BLOCK_TILE_N;        \
      uint32_t dst_ptr =                                                       \
          __cvta_generic_to_shared(&B_sm[SM_LOADING_BUFFER][k][n]);            \
      asm volatile(                                                            \
          "cp.async.ca.shared.global.L2::128B [%0], [%1], 4, 4;\n" ::"r"(      \
              dst_ptr),                                                        \
...
#define test_and_launch_function(function_name)                                \
  template <int BLOCK_TILE_M, int BLOCK_TILE_N, int THREAD_TILE_M,             \
            int THREAD_TILE_N, int TILE_K>                                     \
  void launch_##function_name(const float *A, const float *B, float *C, int M, \
                              int N, int K) {                                  \
    if (N % 128 != 0 || N < 1024) {                                            \
    ...
    static_assert(8 <= BLOCK_TILE_N && BLOCK_TILE_N <= 256 &&                  \
                  ((BLOCK_TILE_N & (BLOCK_TILE_N - 1)) == 0));                 \
    static_assert(8 <= TILE_K && TILE_K <= 128 &&                              \
                  ((TILE_K & (TILE_K - 1)) == 0));                             \
    ...
    dim3 block(host_thread_count_calculator<BLOCK_TILE_M, BLOCK_TILE_N,        \
                                            THREAD_TILE_M, THREAD_TILE_N>());  \
    auto kernel_func = &function_name<BLOCK_TILE_M, BLOCK_TILE_N,              \
                                      THREAD_TILE_M, THREAD_TILE_N, TILE_K>;   \
    auto kSmemSize = 0;                                                        \
    auto err = cudaFuncSetAttribute(                                           \
        kernel_func, cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemSize);  \
    if (err) {                                                                 \
      std::stringstream log_prefix;                                            \
      log_prefix << "function_name=" << #function_name << ", " << BLOCK_TILE_M \
                 << "x" << BLOCK_TILE_N << "x" << TILE_K << ", "               \
                 << THREAD_TILE_M << "x" << THREAD_TILE_N;                     \
      std::cerr << log_prefix.str() << ", " << cudaGetErrorString(err)         \
                << std::endl;                                                  \
      return;                                                                  \
    }                                                                          \
    function_name<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M, THREAD_TILE_N,    \
                  TILE_K><<<grid, block>>>(A, B, C, M, N, K);                  \
  ...
  void test_##function_name(                                                   \
      const float *A, const float *B, float *C, int M, int N, int K,           \
      const std::vector<float> &groundtruth, std::vector<float> &host_C) {     \
    std::stringstream log_prefix;                                              \
    log_prefix << "function_name=" << #function_name << ", " << BLOCK_TILE_M   \
               << "x" << BLOCK_TILE_N << "x" << TILE_K << ", "                 \
               << THREAD_TILE_M << "x" << THREAD_TILE_N;                       \
    ...
    launch_##function_name<BLOCK_TILE_M, BLOCK_TILE_N, THREAD_TILE_M,          \
                           THREAD_TILE_N, TILE_K>(A, B, C, M, N, K);           \
    CHECK_CUDA_ERROR_WITH_INFO(log_prefix.str().c_str());                      \
    ...
    const float(*host_result_ptr)[N] =                                         \
        reinterpret_cast<const float(*)[N]>(groundtruth.data());               \
    ...
    for (int i = 0; i < M; ++i) {                                              \
      for (int j = 0; j < N; ++j) {                                            \
        if (isnan(device_result_ptr[i][j]) ||                                  \
            fabs(host_result_ptr[i][j] - device_result_ptr[i][j]) > EPS) {     \
          printf("%.7f, %.7f\n", host_result_ptr[i][j],                        \
                 device_result_ptr[i][j]);                                     \
          std::stringstream ss;                                                \
          ss << #function_name << ", " << BLOCK_TILE_M << "x" << BLOCK_TILE_N  \
             << ", " << THREAD_TILE_M << "x" << THREAD_TILE_N                  \
    ...
    std::cout << "success, " << #function_name                                 \
              << ",  BLOCK_TILE_M=" << BLOCK_TILE_M                            \
              << ", BLOCK_TILE_N=" << BLOCK_TILE_N << ", TILE_K=" << TILE_K    \
...
test_and_launch_function(
    llmmm__overlap_global2sm2reg__using_if);
test_and_launch_function(
    llmmm__overlap_global2sm2reg__quardra_buffer);
test_and_launch_function(
    llmmm__overlap_global2sm2reg__quardra_buffer__double_ldg_reg);
